...
Suz originally sent me the data, but we should probably get a more authoritative source. 
...
# Legacy pedon data without location info were located on top of Forest Service (FS) buildings when this data was created. Since this is non-sensical these points need to be removed. I tried filtering points by all those that fell inside national forest boundaries, but this excluded ~ 10K observations, many of which were obviously valid pedons, but happened to fall just outisde of the actual boundaries. 

# Remove duplicates. This removes all of the duplicates, but keeps one point at the FS buildings. 
loc2 <- distinct(loc, .keep_all = TRUE)

# Remove all of the coordinates that are not valid locations. I did this by manually reviewing the point dataset on a google satellite hybrid baselayer. It was quite easy to identify locations on FS buildings because they were 1) usually located away from the cluster of points in each forest, and 2) they were in a town (and when I zoomed in far enough I the hybrid baselayer actually labeled the FS building). I suppose that I could have filtered with a shapefile of US towns, but I would rather do this manually to become more familiar with the data points.

TO DO: manually go through the points and ID them. 
...
# Select only those that are within actual National forests because sometimes legacy data was put on top of the field office if no GPS point was recorded.
# Load shapefile of National Forest Service administrative boundaries
fsAdmin <- readOGR("D:/Cascades DSM/Boundaries", layer = "S_USA.AdministrativeForest")
# Reproject to match pedon coordinates
...
# Perform the actual selection (takes ~ 
loc$inNF <- !is.na(over(loc, as(fsAdmin2, "SpatialPolygons")))
loc2 <- loc[loc$inNF == 'TRUE',]
 
writeOGR(loc2, ".", "plotsInForests", driver="ESRI Shapefile")
